/* =====================================================================
   5. GL: the approved shader (GLASS "Deep core", verbatim from the prototype)
   ===================================================================== */
...
var gl = null, prog = null, loc = {}, compTex = null, glLost = false, compCanvas = null;
...
function glInit(){
  try { gl = cv.getContext('webgl', { alpha: true, premultipliedAlpha: true, antialias: false, preserveDrawingBuffer: false }); } catch (e) { gl = null; }
  if (!gl) return false;
  function mk(type, src){ var s = gl.createShader(type); gl.shaderSource(s, src); gl.compileShader(s); if (!gl.getShaderParameter(s, gl.COMPILE_STATUS)){ try { console.warn(gl.getShaderInfoLog(s)); } catch (e) {} return null; } return s; }
  ...
  UNAMES.forEach(function(n){ loc[n] = gl.getUniformLocation(prog, n); });
  compTex = gl.createTexture();
  ...
  gl.uniform1i(loc.uComp, 0);
  gl.clearColor(0, 0, 0, 0);
  return true;
}

/* ---- companion matte: border flood-fill, not a luminance key (black-bodied companions survive) ---- */
var compReady = false, compBg = [0.16, 0.16, 0.17];
function buildMatte(img){
  ...
  var A0 = new Float32Array(N * N), B0 = new Float32Array(N * N);
  for (i = 0; i < N * N; i++) A0[i] = flood[i] ? 0 : 1;
  for (j = 0; j < N; j++) for (i = 0; i < N; i++){ var mn = 1; for (var dy = -1; dy <= 1; dy++) for (var dx = -1; dx <= 1; dx++){ var xx = clamp(i + dx, 0, N - 1), yy = clamp(j + dy, 0, N - 1); mn = Math.min(mn, A0[yy * N + xx]); } B0[j * N + i] = mn; }
  for (var pass = 0; pass < 2; pass++){
    for (j = 0; j < N; j++) for (i = 0; i < N; i++){ var s = 0; for (var ey = -1; ey <= 1; ey++) for (var ex = -1; ex <= 1; ex++){ s += B0[clamp(j + ey, 0, N - 1) * N + clamp(i + ex, 0, N - 1)]; } A0[j * N + i] = s / 9; }
    var t = A0; A0 = B0; B0 = t;
  }
  for (j = 0; j < N; j++) for (i = 0; i < N; i++){
    var rx = (i + 0.5) / N * 2 - 1, ry = (j + 0.5) / N * 2 - 1, rt = Math.sqrt(rx * rx + ry * ry);
    var a = B0[j * N + i] * (1 - sstep(0.70, 0.96, rt)), o2 = (j * N + i) * 4;
    if (a < 0.02){ d[o2] = mr; d[o2 + 1] = mg; d[o2 + 2] = mb; }
    d[o2 + 3] = Math.round(clamp(a, 0, 1) * 255);
  }
  x.putImageData(id, 0, 0);
  compBg = invTone([mr / 255, mg / 255, mb / 255]);
  return c;
}
/* the matte canvas is kept, so a restored GL context can re-upload it */
function uploadComp(){
  if (!gl || glLost || !compCanvas) return;
...
/* ---- companions: art from the injected companions.json (the ONLY art source), identity from session() ---- */
var COMPANIONS = (function(){ try { var n = $('companions'); var a = JSON.parse(n ? n.textContent : '[]'); return Array.isArray(a) ? a : []; } catch (e) { return []; } })();
function artFor(webId){ for (var i = 0; i < COMPANIONS.length; i++) if (COMPANIONS[i] && COMPANIONS[i].id === webId) return COMPANIONS[i]; return null; }
function titleCase(s){ s = String(s || ''); return s.charAt(0).toUpperCase() + s.slice(1).toLowerCase(); }
/* ME: the chosen companion (null before the pick; the glass then runs with no tint and no companion) */
var ME = { id: null, webId: null, art: null, label: '', pal: 'ghost', tint: hex(GLASS.ghost), cap: 0 };
var compLoadSeq = 0, compLoadP = null;
function loadCompanionArt(){
  var seq = ++compLoadSeq, art = ME.art;
  compReady = false; compCanvas = null;
  try { $('fbImg').src = art && art.dataUri ? art.dataUri : ''; } catch (e) {}
  if (!art || !art.dataUri){ compLoadP = Promise.resolve(false); return compLoadP; }
  compLoadP = new Promise(function(res){
    var img = new Image();
    img.onload = function(){
      if (seq !== compLoadSeq) return res(false);
      try { compCanvas = buildMatte(img); uploadComp(); compReady = true; dirty = true; } catch (e) { compReady = false; }
      res(compReady);
    };
