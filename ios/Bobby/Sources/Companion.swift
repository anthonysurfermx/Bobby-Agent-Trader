...
let bobbyCompanions: [Companion] = [
    ...
    .init(id: "kora", label: "KORA", role: L.t("CONVERSATION", "CONVERSACIÓN"),
          personality: L.t("talks markets like your best friend", "platica del mercado como tu mejor amiga"),
    ...
    .init(id: "glitch", label: "GLITCH", role: L.t("RED TEAM", "ABOGADO DEL DIABLO"),
    ...
    .init(id: "axiom", label: "AXIOM", role: L.t("TRACK RECORD", "HISTORIAL"),
    ...
    // ---- Wave 2 (2026-09-07) — mirrors src/lib/companions/data.ts exactly.
    // Voices come from FEM_VOICES in api/_lib/tts.ts so the gendered Spanish
    // delivery matches. See docs/brand/bobby-character-bible-wave2.md.
    .init(id: "iris", label: "IRIS", role: L.t("MARKET REGIME", "RÉGIMEN DE MERCADO"),
          personality: L.t("reads the weather of the whole market", "lee el clima de todo el mercado"),
          selectLine: L.t("Let me read the weather first. Today there is wind.", "Déjame leer el clima primero. Hoy hay viento."),
          secretPhrase: L.t("The regime decides more than the setup does.", "El régimen decide más que la entrada."),
          hue: 0.545, requiredLevel: 1,
          voicePersona: "sage",
          evolutionNames: ["IRIS", "IRIS CLEAR", "IRIS FRONT", "IRIS STORM", "IRIS HORIZON"]),
    .init(id: "sol", label: "SOL", role: L.t("GROWTH", "CRECIMIENTO"),
          personality: L.t("celebrates every step you actually take", "celebra cada paso que sí das"),
          selectLine: L.t("Let us build your world, one earned piece at a time.", "Vamos a construir tu mundo, pieza ganada por pieza ganada."),
          secretPhrase: L.t("Discipline levels you up. Deposits never did.", "La disciplina te sube de nivel. Los depósitos nunca lo hicieron."),
          hue: 0.115, requiredLevel: 1,
          voicePersona: "coral",
          evolutionNames: ["SOL", "SOL SPROUT", "SOL BRANCH", "SOL CANOPY", "SOL BLOOM"]),
    .init(id: "zuri", label: "ZURI", role: L.t("ON-CHAIN FLOWS", "FLUJOS ON-CHAIN"),
          personality: L.t("follows the money and tells you where it went", "sigue el dinero y te dice a dónde se fue"),
          selectLine: L.t("Give me a minute. I am following the thread.", "Dame un minuto. Voy siguiendo el hilo."),
          secretPhrase: L.t("Wallets do not post. They just move.", "Las wallets no publican. Solo se mueven."),
          hue: 0.075, requiredLevel: 1,
          voicePersona: "nova",
          evolutionNames: ["ZURI", "ZURI TRAIL", "ZURI SIGNAL", "ZURI DEPTH", "ZURI ORACLE"]),
    .init(id: "mira", label: "MIRA", role: L.t("PRACTICE", "PRÁCTICA"),
          personality: L.t("zero pressure, celebrates the attempt", "cero presión, celebra el intento"),
          selectLine: L.t("No money on the line yet. Let us just look.", "Todavía sin dinero de por medio. Nomás vamos viendo."),
          secretPhrase: L.t("A rehearsal costs nothing. A habit pays forever.", "Un ensayo no cuesta nada. Un hábito paga siempre."),
          hue: 0.505, requiredLevel: 1,
          voicePersona: "alloy",
          evolutionNames: ["MIRA", "MIRA DRILL", "MIRA PACE", "MIRA FORM", "MIRA STAR"]),
    .init(id: "nalu", label: "NALU", role: L.t("LIQUIDITY", "LIQUIDEZ"),
          personality: L.t("reads flow like a wave, everything is timing", "lee el flujo como una ola, todo es saber cuándo"),
          ...
          hue: 0.075, requiredLevel: 1,
          voicePersona: "marin",
          evolutionNames: ["NALU", "NALU SWELL", "NALU CREST", "NALU BARREL", "NALU TIDE"]),
    .init(id: "vega", label: "VEGA", role: L.t("PROBABILITY", "PROBABILIDAD"),
          personality: L.t("gives you the odds, never the promise", "te da la probabilidad, nunca la promesa"),
          selectLine: L.t("I can give you the probability. Not the promise.", "Te puedo dar la probabilidad. La promesa no."),
          secretPhrase: L.t("A number without its error bar is a story.", "Un número sin su margen de error es un cuento."),
          hue: 0.115, requiredLevel: 3,
          voicePersona: "shimmer",
          evolutionNames: ["VEGA", "VEGA SIGMA", "VEGA CONE", "VEGA PRIME", "VEGA ABSOLUTE"]),
    .init(id: "noor", label: "NOOR", role: L.t("MENTORSHIP", "MENTORÍA"),
          personality: L.t("unhurried, firm, has seen every cycle", "sin prisa, firme, ya vio todos los ciclos"),
          selectLine: L.t("Sit down. Let us review the week before the next trade.", "Siéntate. Revisemos la semana antes de la próxima operación."),
          secretPhrase: L.t("I have seen this cycle before. It ended the same way.", "Ya vi este ciclo antes. Terminó igual."),
          hue: 0.115, requiredLevel: 4,
          voicePersona: "fable",
          evolutionNames: ["NOOR", "NOOR RING", "NOOR CIRCLE", "NOOR CROWN", "NOOR ETERNAL"]),
    .init(id: "keo", label: "KEO", role: L.t("PATIENCE", "PACIENCIA"),
          personality: L.t("never in a hurry, and never early", "nunca con prisa, y nunca antes de tiempo"),
...
enum CompanionEmote: String, CaseIterable, Identifiable {
    ...
    var label: String {
        switch self {
        case .pulse: return L.t("PULSE", "PULSO")
        case .orbit: return L.t("ORBIT", "ÓRBITA")
        case .victory: return L.t("VICTORY", "VICTORIA")
        case .shield: return L.t("SHIELD", "ESCUDO")
        case .legend: return L.t("LEGEND", "LEYENDA")
        }
    }
...
final class CompanionStore: ObservableObject {
    ...
    func applyServer(_ server: ServerProgress, acknowledged: [String]) {
        ...
        let firstReconcile = syncedAt == nil
        ...
        // First reconcile for this owner (new phone, sign-in, account switch):
        // what the account already had is not "new" in the locker. Drops that
        // arrive in later syncs still are.
        if firstReconcile { LockerSeen.mark(LockerLedger.ownedIds(ownId: companionId, xp: server.xp), defaults: defaults) }
        syncedAt = Date()
    }
    ...
    /// What one award granted: the points the cap allowed and the queued
    /// event id the server answers about (nil when the cap rejected it).
    struct DisciplineAward: Equatable { let points: Int; let eventID: String? }
    ...
    @discardableResult
    func awardDiscipline(_ points: Int, kind: String = "read_complete", thesis: AwardThesis? = nil, now: Date = Date()) -> Int {
        awardDisciplineEvent(points, kind: kind, thesis: thesis, now: now).points
    }

    /// Same award, also returning the queued event id so the desk can follow
    /// the Trader Land piece the server plants for it.
    @discardableResult
    func awardDisciplineEvent(_ points: Int, kind: String = "read_complete", thesis: AwardThesis? = nil, now: Date = Date()) -> DisciplineAward {
        ...
        if let day = defaults.object(forKey: Key.dailyAwardsDay) as? Date,
           cal.isDate(day, inSameDayAs: now) {
            guard dailyCount < maxDailyAwards else { return DisciplineAward(points: 0, eventID: nil) }
            dailyCount += 1
        } else {
            dailyCount = 1
        }
        ...
        pendingAwards.append(PendingAward(id: eventID, kind: kind, at: ISO8601DateFormatter().string(from: now), tzOffsetMin: -TimeZone.current.secondsFromGMT(for: now) / 60, thesis: thesis))
        return DisciplineAward(points: points, eventID: eventID)
    }
}
