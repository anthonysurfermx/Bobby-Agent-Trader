...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!await enforcePublicRateLimit(req, res, 'okx-onchain', 60, 600)) return;

  const { action, chainIndex = '1', tokenAddress, limit = '20' } = req.query;
  if (!/^\d{1,10}$/.test(String(chainIndex)) || !/^\d{1,3}$/.test(String(limit))
    || Number(limit) < 1 || Number(limit) > 100
    || (tokenAddress && !/^0x[a-fA-F0-9]{40}$/.test(String(tokenAddress)))) {
  ...
  try {
    ...
    return res.status(500).json({ error: 'OKX data request failed' });
  }
}
