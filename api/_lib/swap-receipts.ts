...
export async function recordBuiltSwap(row: BuiltSwapRow, fetchImpl: typeof fetch = fetch): Promise<{ recorded: boolean; reason?: string }> {
  if (!bobbyDbConfigured()) return { recorded: false, reason: 'database not configured' };
  try {
    const res = await fetchImpl(bobbyRest('swap_receipts?on_conflict=wallet_address,calldata_hash'), {
      method: 'POST',
      ...
      body: JSON.stringify({
        ...
        quoted_out: row.quotedOut,
        min_out: row.minOut,
        route: row.route,
        router: row.router.toLowerCase(),
...
type ReceiptClient = Pick<ReturnType<typeof baseClient>, 'getTransaction' | 'getTransactionReceipt'>;
...
export async function verifySwapOnChain(txHash: Hex, wallet: Address, client: ReceiptClient = baseClient()): Promise<ChainVerification> {
  const base: ChainVerification = { ok: false, reason: null, txHash, from: null, to: null, status: null, blockNumber: null, calldataHash: null, valueWei: null, movements: [], amountInRaw: null, amountOutRaw: null };
  let tx; let receipt;
  try {
...
export async function confirmSwapReceipt(v: ChainVerification, wallet: string, fetchImpl: typeof fetch = fetch): Promise<ConfirmResult> {
  ...
  try {
    const q = `swap_receipts?wallet_address=eq.${wallet.toLowerCase()}&calldata_hash=eq.${v.calldataHash}&select=id,status,tx_hash`;
    const found = await fetchImpl(bobbyRest(q), { headers });
    if (!found.ok) return { recorded: false, outcome: 'db_error', id: null };
    const rows = (await found.json()) as Array<{ id: string; status: string; tx_hash: string | null }>;
    ...
    const patch = await fetchImpl(bobbyRest(`swap_receipts?id=eq.${row.id}&status=eq.built`), {
      method: 'PATCH',
      headers: { ...headers, Prefer: 'return=representation' },
      body: JSON.stringify({
        status: 'confirmed',
        tx_hash: v.txHash.toLowerCase(),
        block_number: v.blockNumber ? Number(v.blockNumber) : null,
        amount_in_raw: v.amountInRaw,
    ...
    if (!updated.length) return confirmSwapReceipt(v, wallet, fetchImpl);
