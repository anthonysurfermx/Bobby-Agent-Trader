...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const body = (req.body || {}) as { agentId?: string; newOwner?: string; expectedVersion?: number; requestId?: string };
  ...
  if (!Number.isInteger(body.expectedVersion) || Number(body.expectedVersion) < 1) return res.status(400).json({ error: 'expectedVersion must be the row version you read' });
  ...
  // Signed by the CURRENT owner over this exact payload (agentId, newOwner, expectedVersion, requestId).
  ...
  const result = await transferAgentOwner(String(body.agentId), String(existing.owner_address), String(body.newOwner), Number(body.expectedVersion), String(body.requestId));
  if (!result.ok) {
  ...
  return res.status(200).json({ ok: true, agentId: result.row.agent_id, owner: result.row.owner_address, version: result.row.version });
}
