...
import { APPROVED_TABLES, IDENTITY_TABLES } from './tables.js';
...
interface M { ref: string; allowMissing?: boolean; tables: Array<{ table: string; exists: boolean; rows: number; sha256: string | null; proofs?: Record<string, Proof> }> }
...
(async () => {
  ...
  // Orphans on the target: every FK value must exist in its parent.
  for (const t of APPROVED_TABLES) {
    for (const fk of t.fks) {
      ...
      for await (const page of rows<Record<string, unknown>>(p, parent, [parentCol], parentCol)) for (const r of page) parentKeys.add(String(r[parentCol]));
      let orphans = 0; let total = 0;
      for await (const page of rows<Record<string, unknown>>(p, t.name, t.pk, `${t.pk.join(',')},${col}`)) for (const r of page) { if (r[col] === null || r[col] === undefined) continue; total += 1; if (!parentKeys.has(String(r[col]))) orphans += 1; }
  ...
  if (expectOutbox) {
    const st = await rpc<Array<{ table_name: string }>>(p, 'bobby_outbox_status');
    const covered = new Set(Array.isArray(st.body) ? st.body.map((x) => x.table_name) : []);
    const expected = new Set(APPROVED_TABLES.map((t) => t.name));
    const missing = [...expected].filter((x) => !covered.has(x)); const extra = [...covered].filter((x) => !expected.has(x));
    line(st.status === 200 && missing.length === 0 && extra.length === 0, `outbox triggers cover exactly the ${expected.size} approved tables`, `covered=${covered.size} missing=[${missing.join(',')}] extra=[${extra.join(',')}]`);
  }
