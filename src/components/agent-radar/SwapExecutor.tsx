// ============================================================
// SwapExecutor — Coinbase B20 tokenized stocks on Base via Uniswap V3
//
...
type SwapStep = 'idle' | 'quoting' | 'quoted' | 'approving' | 'requoting' | 'swapping' | 'verifying' | 'confirmed' | 'unrecorded' | 'error';
...
export function SwapExecutor({ defaultFrom = 'USDC', defaultTo = 'NVDAc', className = '' }: Props) {
  ...
  const fetchQuote = useCallback(async (eligible: boolean): Promise<SwapQuoteView> => {
    if (!address || !from || !to) throw new Error('Connect a wallet and pick a pair');
    const headers = await sessionHeaders();
    const res = await fetch('/api/base-swap', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...headers },
      body: JSON.stringify({ tokenIn: from.symbol, tokenOut: to.symbol, amount: amount.trim(), slippagePct: parseFloat(slippage), wallet: address.toLowerCase(), stockEligibilityConfirmed: eligible }),
    });
  ...
  const notExpired = (q: SwapQuoteView) => {
    if (q.tx && q.tx.deadline <= Math.floor(Date.now() / 1000) + 15) throw new Error('Quote expired. Get a fresh quote before signing.');
  };

  // ---- Step 1: quote (+ calldata when the guards pass and the human attested eligibility) ----
  const getQuote = useCallback(async () => {
    setStep('quoting'); setError(null); setQuote(null); setTxHash(undefined); setReceiptNote(null);
    try {
  ...
  const submitReceipt = useCallback(async (hash: Hex) => {
    ...
    for (let attempt = 0; attempt < 6; attempt++) {
      const res = await fetch('/api/swap-receipt', { method: 'POST', headers: { 'Content-Type': 'application/json', ...headers }, body: JSON.stringify({ txHash: hash, wallet: address.toLowerCase(), platform: 'web' }) });
      const data = await res.json().catch(() => ({}));
      if (res.status === 202) { await new Promise((r) => setTimeout(r, 2000 * (attempt + 1))); continue; }
      if (res.ok && data.ok) { setReceiptNote(`Verified on ${BASE.name} and recorded (${data.receipt?.outcome})`); setStep('confirmed'); return; }
  ...
  const stock = quote?.stockReference ?? null;
    ...
    <div className={`space-y-3 ${className}`}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="text-[10px] font-bold text-green-400">TOKENIZED STOCKS</span>
          <span className="text-[9px] text-neutral-600">Coinbase B20 · Uniswap V3 on {BASE.name}</span>
        </div>
        {isConnected && (
      ...
      {isConnected && step !== 'confirmed' && step !== 'unrecorded' && (
        <>
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1 bg-neutral-900/60 border border-neutral-800 rounded-xl p-3">
              ...
              <div className="flex items-center gap-2">
                <span className="flex-1 text-lg font-bold text-neutral-100">
                  {quote ? Number(quote.amountOut).toLocaleString(undefined, { maximumFractionDigits: 6 }) : '—'}
                </span>
                <select value={toToken} onChange={e => { setToToken(e.target.value); if (e.target.value !== 'USDC') setFromToken('USDC'); reset(); }} className="bg-neutral-800 text-neutral-200 text-sm font-medium rounded-lg px-2 py-1 outline-none border border-neutral-700" disabled={busy}>
                  {PAIR_TOKENS.filter(t => t !== fromToken).map(t => <option key={t} value={t}>{t}</option>)}
                </select>
              </div>
            </div>
          </div>
          ...
          <label className="flex items-start gap-2 text-[11px] text-neutral-400 cursor-pointer">
            <input type="checkbox" className="mt-0.5" checked={acknowledged} onChange={(ev) => { setAcknowledged(ev.target.checked); if (step === 'quoted') reset(); }} disabled={busy} />
            <span>I am in an eligible jurisdiction outside the U.S. I understand a B20 token is not the underlying share, and I will check the contract, the chain and the minimum received before signing.</span>
          </label>

          {quote && (
            <div className="bg-neutral-900/40 border border-green-500/15 rounded-xl p-3 space-y-1.5 text-[10px]">
              <div className="flex justify-between"><span className="text-neutral-500">Rate</span><span className="text-neutral-300">1 {fromToken} = {quote.executionPrice.toLocaleString(undefined, { maximumFractionDigits: 8 })} {toToken}</span></div>
              ...
              {needsApproval ? (
                <>
                  ...
                  <div className="text-amber-300/80">The swap calldata arrives after the approval mines and the server re-simulates. If you stop after approving, or the swap reverts, the allowance stays until spent or revoked.</div>
                </>
              ) : (
          ...
          <div className="space-y-2">
            ...
            {step === 'quoting' && <div className="w-full py-3 flex items-center justify-center gap-2 text-sm text-neutral-400"><Loader2 className="w-4 h-4 animate-spin" /> Checking Uniswap and the B20 reference...</div>}
      ...
      {step === 'unrecorded' && txHash && (
        <div className="space-y-3">
          <div className="flex items-start gap-2 bg-amber-500/10 border border-amber-500/20 rounded-xl p-3">
            <AlertCircle className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
            <div>
              <div className="text-xs text-amber-400 font-medium">Mined on {BASE.name}, not recorded</div>
              {receiptNote && <div className="text-[10px] text-neutral-400 mt-0.5">{receiptNote}</div>}
            </div>
          </div>
          <button onClick={() => { setStep('verifying'); void submitReceipt(txHash).catch((e) => { setStep('unrecorded'); setReceiptNote(e instanceof Error ? e.message : 'retry failed'); }); }} className={primary}>Retry record</button>
          {explorerUrl && (
