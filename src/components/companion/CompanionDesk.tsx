...
function summary(a: Answer): string {
  ...
  if (a.direction && a.convictionPct !== null) { const d = a.direction === 'long' ? t('bullish', 'alcista') : a.direction === 'short' ? t('bearish', 'bajista') : a.direction; lines.push(t(`My read: ${d} bias with ${Math.round(a.convictionPct)}% conviction.`, `Mi lectura: sesgo ${d} con ${Math.round(a.convictionPct)}% de convicción.`)); }
        ...
        <div className="rounded-2xl p-4 bg-white/[0.02] border border-white/[0.05] space-y-3">
          ...
          <div className="rounded-xl border border-white/[0.05] p-3">
            ...
            <div className="mt-2 grid grid-cols-3 gap-2 text-[11px] font-mono">
              {[['ALPHA', '#34D399', answer.trend ? t(`trend ${localizedTrend(answer.trend)}`, `tendencia ${localizedTrend(answer.trend)}`) : '…'], ['RED TEAM', '#f87171', answer.stop !== null ? t(`invalidates ${money(answer.stop)}`, `invalida ${money(answer.stop)}`) : answer.support !== null ? t(`support ${money(answer.support)}`, `soporte ${money(answer.support)}`) : '…'], ['CIO', '#F5C542', answer.direction && answer.convictionPct !== null ? `${answer.direction} ${Math.round(answer.convictionPct)}%` : t('decides', 'decide')]].map(([name, color, line]) => (
