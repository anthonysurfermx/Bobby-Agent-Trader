    ...
    <KineticShell minimalNav>
      ...
      <div className="mx-auto w-full max-w-7xl px-4 pb-24 text-[#ecf5e9] sm:px-6">
        <section className="pt-6 sm:pt-10">
          ...
          <p className="mt-4 max-w-2xl text-sm leading-relaxed text-emerald-50/65 sm:text-base">{t('Visit islands shared by their builders. Look at how they connect paths, leave space and combine districts, then bring an idea back to your own island.', 'Visita islas compartidas por sus creadores. Mira cómo conectan caminos, dejan espacio y combinan distritos; después lleva una idea a tu propia isla.')}</p>
        ...
        <section id="comunidad" className="mt-16 scroll-mt-24" aria-labelledby="community-title">
          <div className="flex items-end justify-between gap-4">
            <div><span className="font-mono text-[10px] uppercase tracking-[0.2em] text-emerald-200/65">01</span><h2 id="community-title" className="mt-1 text-2xl font-semibold tracking-tight sm:text-3xl">{t('Community islands', 'Islas de la comunidad')}</h2></div>
        ...
        <section id="mundos" className="mt-14" aria-labelledby="worlds-title">
          <div className="flex items-end justify-between gap-4">
            <div><span className="font-mono text-[10px] uppercase tracking-[0.2em] text-emerald-200/65">02</span><h2 id="worlds-title" className="mt-1 text-2xl font-semibold tracking-tight sm:text-3xl">{t('The five worlds', 'Los cinco mundos')}</h2></div>
            <p className="hidden max-w-xs text-right text-xs text-emerald-50/50 sm:block">{t('Discover the pieces of each district, plus the Aura Core every island starts with.', 'Descubre las piezas de cada distrito y el Aura Core con el que empieza toda isla.')}</p>
          </div>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {DISTRICTS.map((district) => {
              ...
              const pieces = (manifest?.items ?? []).filter((item) => item.district === district && !(item.id in CATALOG_ALIASES)).sort((a, b) => KIND_ORDER.indexOf(a.kind) - KIND_ORDER.indexOf(b.kind));
              return (
